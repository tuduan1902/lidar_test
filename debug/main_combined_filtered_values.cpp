 ...
 * Combined LiDAR + Ultrasonic occupancy grid with distance display.
 * This file adds live display of LiDAR distance/angle and four ultrasonic
 * values, while preserving the filtered grid and not modifying existing files.
 *
 * Build:
...
class FilteredCombinedValuesManager {
public:
    FilteredCombinedValuesManager(const std::string& lidar_dev,
        ...
        , us_baud_(us_baud) {
        for (int i = 0; i < 4; i++) last_us_cm_[i].store(-1.0f);
    }
    ...
    uint16_t last_lidar_dist_mm() const { return last_lidar_dist_mm_.load(); }
    int32_t last_lidar_angle_tenths() const { return last_lidar_angle_tenths_.load(); }
    float last_us_cm(int i) const { return last_us_cm_[i].load(); }
...
private:
    ...
    std::atomic<uint16_t> last_lidar_dist_mm_{0xFFFF};
    std::atomic<int32_t> last_lidar_angle_tenths_{0};
...
bool FilteredCombinedValuesManager::uart_open(const std::string& dev, int baud, int& fd) {
    char cmd[160];
...
void FilteredCombinedValuesManager::uart_close(int& fd) {
    if (fd >= 0) { ::close(fd); fd = -1; }
}

bool FilteredCombinedValuesManager::parse_lidar(const uint8_t* b, LidarPoint& o) {
...
bool FilteredCombinedValuesManager::parse_us_json(const char* line, UsPoint& o) {
...
void FilteredCombinedValuesManager::mark_xy(float wx, float wy, uint8_t weight, uint8_t src) {
...
void FilteredCombinedValuesManager::mark_lidar(const LidarPoint& p) {
...
void FilteredCombinedValuesManager::mark_us(const UsPoint& p) {
    for (int i = 0; i < 4; i++) {
        ...
        mark_xy(wx, wy, 2, (uint8_t)(2 + i));
    }
}

void FilteredCombinedValuesManager::lidar_reader_loop() {
    ...
    while (running_.load(std::memory_order_relaxed)) {
        ...
        if (parse_lidar(pkt, lp)) {
            pkts++;
            last_lidar_dist_mm_.store(lp.dist_mm, std::memory_order_relaxed);
            last_lidar_angle_tenths_.store((int32_t)std::lround(lp.angle_deg * 10.0f),
...
void FilteredCombinedValuesManager::us_reader_loop() {
        ...
        if (b == '\n' || b == '\r') {
            if (lpos > 5) {
                ...
                if (parse_us_json(line, up)) {
                    pkts++;
                    for (int i = 0; i < 4; i++) last_us_cm_[i].store(up.dist_cm[i], std::memory_order_relaxed);
                    if (pkts <= 3) {
...
void FilteredCombinedValuesManager::updater_loop() {
...
void FilteredCombinedValuesManager::decay_loop() {
...
void FilteredCombinedValuesManager::start() {
    ...
    running_.store(true);
    thr_lidar_r_ = std::thread(&FilteredCombinedValuesManager::lidar_reader_loop, this);
    thr_us_r_    = std::thread(&FilteredCombinedValuesManager::us_reader_loop,    this);
    thr_updater_ = std::thread(&FilteredCombinedValuesManager::updater_loop,    this);
    thr_decay_   = std::thread(&FilteredCombinedValuesManager::decay_loop,      this);
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
}

void FilteredCombinedValuesManager::stop() {
...
static void draw(const uint8_t* g, const uint8_t* src,
                 uint32_t lidar_pts, uint32_t us_pts, float hz,
                 uint16_t last_lidar_dist_mm, int32_t last_lidar_angle_tenths,
                 const std::array<float,4>& last_us) {
    constexpr int Vx  = GRID_N;
    constexpr int Vy  = GRID_N;
    constexpr int offx = 0;
    constexpr int offy = 0;

    std::printf("\033[2J\033[H");
    std::printf("  LiDAR+US Filtered Grid  |  lidar=%-6u  us=%-6u  %.0f pts/s\n",
                lidar_pts, us_pts, hz);
    std::printf("  LiDAR: %s\n",
                last_lidar_dist_mm == 0xFFFF ? "distance=--" : "distance=" );
    if (last_lidar_dist_mm != 0xFFFF) {
        float meters = last_lidar_dist_mm / 1000.0f;
        float angle = last_lidar_angle_tenths / 10.0f;
        std::printf("  LiDAR: %4d mm (%.2fm)  goc=%5.1f deg\n",
                    last_lidar_dist_mm, meters, angle);
    } else {
        std::printf("  LiDAR: invalid\n");
    }
    std::printf("  US cm: [ %5.1f | %5.1f | %5.1f | %5.1f ]\n",
                last_us[0], last_us[1], last_us[2], last_us[3]);
    std::printf("  [X]=xe  [L]=LiDAR  [1..4]=US1..US4\n");
    std::printf("  [1,2]=US trai  [3,4]=US phai\n");
    std::printf("  [*]=vat_can manh  [.] = vat_can nhe  [ ]=trong\n");
...
int main(int argc, char** argv) {
    ...
    std::printf("=== LiDAR + Ultrasonic Filtered Values Grid ===\n");
    std::printf("LiDAR  : %s @ 115200\n", lidar_dev);
    std::printf("US     : %s @ 115200\n\n", us_dev);
    fflush(stdout);

    FilteredCombinedValuesManager mgr(lidar_dev, us_dev);
    mgr.start();
    ...
    while (!g_quit.load(std::memory_order_relaxed)) {
        ...
        std::array<float,4> last_us_vals;
        for (int i = 0; i < 4; i++) last_us_vals[i] = mgr.last_us_cm(i);

        draw(snap, src,
             lp, up, hz,
             mgr.last_lidar_dist_mm(), mgr.last_lidar_angle_tenths(),
             last_us_vals);
